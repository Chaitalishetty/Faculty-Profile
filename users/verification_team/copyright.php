...
$i=0;
$output="";
$sql =  "SELECT * FROM copyright where `verified`='1' " ; 
...
while($row = mysqli_fetch_array($result)){
  ...
  $output.="<tr><th scope='row'>".$i."</th>
            ...
            <td>".$row["reg_date"]."</td>
            <td><a href='verify\pub_copyright.php?id=".$row["id"]."' class='icon-block'><b><i class='far fa-file-alt'></i></b></a></td></tr>";
}
  ...
  $select="SELECT * FROM copyright";
   if(isset($_POST["date_from"]) &&$_POST["date_to"] && $_POST["date_from"]!="" && $_POST["date_to"]!=""){
     $from=date('Y-m-d',strtotime($_POST['date_from']));
       ...
       $filter_query.=" WHERE `verified`='1' and reg_date between '$from' and '$to'";
