...
while($row = mysqli_fetch_array($result)){
  ...
  $output.="<tr><th scope='row'>".$i."</th>
            ...
            <td>".$row["organiser"]."</td>
            <td>".$row["org_type"]."</td>
            <td><a href='verify\par_work_att.php?id=".$row["Srno"]."' class='icon-block'><b><i class='far fa-file-alt'></i></b></a></td></tr>";
}
  ...
  $select="SELECT * FROM workshop";
   if(isset($_POST["date_from"]) &&$_POST["date_to"] && $_POST["date_from"]!="" && $_POST["date_to"]!=""){
     $from=date('Y-m-d',strtotime($_POST['date_from']));
       ...
       $filter_query.=" WHERE `verified`='1' and sdate between '$from' and '$to'";
