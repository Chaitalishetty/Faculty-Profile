...
$i=0;
$output="";
$sql =  "SELECT * FROM book_publication where `verified`='1' " ; 
...
while($row = mysqli_fetch_array($result)){
  ...
  $output.="<tr><th scope='row'>".$i."</th>
            ...
            <td>".$row["isbn_no"]."</td>
            <td>".$row["year"]."</td>
            <td><a href='verify\pub_book.php?id=".$row["id"]."' class='icon-block'><b><i class='far fa-file-alt'></i></b></a></td></tr>";
}
  ...
  $select="SELECT * FROM book_publication";
   if(isset($_POST["date_from"]) &&$_POST["date_to"] && $_POST["date_from"]!="" && $_POST["date_to"]!=""){
     $from=date('Y-m-d',strtotime($_POST['date_from']));
       ...
       $filter_query.=" WHERE `verified`='1' and year between '$from' and '$to'";
